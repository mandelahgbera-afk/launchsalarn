...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Mirror user state in a ref so callbacks always read the latest value (avoids stale closures)
  ...
  // Keep userRef in sync with user state so callbacks never see stale user value
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, newSession) => {
      ...
      // Use userRef.current (not closure-captured user) to avoid stale null check
      if (
      ...
      // Skip re-fetch on SIGNED_IN when the same user is already loaded
      // (e.g., after supabase.auth.verifyOtp re-authenticates during withdrawal)
      if (
        event === 'SIGNED_IN' &&
        lastLoadedAuthId.current === newSession.user.id &&
        userRef.current !== null
  ...
  const signIn = async (email: string, password: string) => {
    try {
      ...
      if (!error && data?.user) {
        ...
        // Ensure balance row exists for this user (idempotent, handles edge cases
        // where the row was never created due to trigger not running)
        supabase.from('user_balances').upsert(
